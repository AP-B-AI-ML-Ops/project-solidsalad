...
def calculate_metrics(current_data, dv, ref_data):
    ...
    current_df = pd.DataFrame(
        current_data_transformed.toarray(), columns=dv.feature_names_
    )
    ...
    report.run(
        reference_data=ref_df, current_data=current_df, column_mapping=COL_MAPPING
    )

    result = report.as_dict()

    num_drifted_cols = result["metrics"][0]["result"]["number_of_drifted_columns"]
    share_missing_vals = result["metrics"][1]["result"]["current"][
    ...
    with psycopg.connect(f"{CONNECT_STRING} dbname=test") as conn:
        with conn.cursor() as cursor:
            ...
            for i in range(0, 2700, 100):  # Assuming chunk size of 100 for simulation
                ...
                num_drifted_cols, share_missing_vals = calculate_metrics(
                    current_data, dv, ref_data
                )
                save_metrics_to_db(
                    cursor, startDate, num_drifted_cols, share_missing_vals
                )
